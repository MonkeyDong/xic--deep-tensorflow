...
X_tr = np.load("save_npy/x_train.npy") #(n,向量长度，通道数) (n,256,32)
X_vld = np.load("save_npy/x_ver.npy") 
y_tr = fun(np.load("save_npy/y_train.npy")) #one_hot编码向量 (n,)
y_vld =  fun(np.load("save_npy/y_ver.npy"))

X_test = np.load("save_npy/X_test.npy")
y_test = fun(np.load("save_npy/y_test.npy"))
...
with tf.Session(graph=graph) as sess:
    ...
    t = sys.argv[1]
    np.save("output/output"+t+".npy",logit)
    np.save("output/labels"+t+".npy",y_test)
    np.save("output/y_pres.npy",y_pres)
